...
function _smf28_ref_fiber()
    return FiberCrossSection(
        GermaniaSilicaGlass(0.036),   # 3.6 mol % GeO₂ core
        GermaniaSilicaGlass(0.0),     # pure silica cladding
        8.2e-6, 125e-6;
...
@testset "silica_nonlinear" begin
    ...
    λ_ref   = 1550e-9
    T_ref   = 297.15
    γ_SMF   = nonlinear_coefficient(XS, λ_ref, T_ref)
    ...
    if DEBUG_PLOTS && _PLOTS_OK
        try
            ...
            outfile = joinpath(@__DIR__, "debug_raman_summary_stats.png")
