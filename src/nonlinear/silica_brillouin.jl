"""
silica_brillouin.jl — Spontaneous Brillouin scattering noise in **fused silica
fibre**.

Included into the `Nonlinear` submodule of `Bifrost` by `Bifrost.jl`,
after `silica_raman.jl`. Reuses `_simpson` / `_trapz` from `silica_raman.jl`
and `SPEED_OF_LIGHT_M_PER_S` from `material-properties.jl`, both pulled in
earlier within the same submodule.
...
Pure fused SiO₂ is the reference material: the measured peak gain
`BRIL_G_PEAK_SiO2_1550 = 5×10⁻¹¹ m/W` and the empirical SMF-28 loss model
both anchor at undoped silica. SiO₂–GeO₂ doping is supported through linear
mass-weighted bulk mixing rules for ρ and C₁₁ (textbook values, no fitted
coefficients), which propagate through `acoustic_velocity_bulk`,
`brillouin_freq_shift`, and `g_B_peak`. The opto-acoustic overlap correction
— the dominant cause of measured g_B(SMF-28) ≈ 2.2×10⁻¹¹ m/W versus the bulk
formula's ~5×10⁻¹¹ m/W — is *not* modelled here; see the inline note above
`g_B_peak`. For heavily doped fibres, pass a measured `g_B_peak` directly.
...
to be `SilicaGermaniaGlass` (so the mixing-rule v_A formulas apply). Extending
...
function acoustic_mode_terms(xs::FiberCrossSection, λ_pump::Real, T_K::Real;
    ...
    (xs.core_material isa SilicaGermaniaGlass) &&
    (xs.cladding_material isa SilicaGermaniaGlass) ||
        throw(ArgumentError(
            "acoustic_mode_terms currently requires SilicaGermaniaGlass for " *
...
function g_B_peak_GeO2(fiber::Fiber; λ_pump::Real=1550e-9,
    ...
    core isa SilicaGermaniaGlass ||
        throw(ArgumentError("g_B_peak_GeO2(::Fiber) requires a SilicaGermaniaGlass core; got $(typeof(core))"))
...
# ═══════════════════════════════════════════════════════════════════════════
# Thermal phonon occupancy (Brillouin band — same physics as the Raman
# version in silica_raman.jl, separate name to avoid signature collision
# with the differing internal unit conventions there)
# ═══════════════════════════════════════════════════════════════════════════
...
function brillouin_threshold(fiber::Fiber, λ_pump; T_K=fiber.T_ref_K,
    ...
    g_B   = fiber.cross_section.core_material isa SilicaGermaniaGlass ?
...
function spbs_noise_in_channel(fiber::Fiber, λ_pump, λ_channel, Δλ, P_pump;
    ...
    L     = length_m === nothing ? fiber.s_end - fiber.s_start : length_m
    n_eff = effective_mode_index(xs, λ_pump, T_K)
    m_Ge  = xs.core_material isa SilicaGermaniaGlass ? xs.core_material.x_ge : 0.0
    g_B   = xs.core_material isa SilicaGermaniaGlass ? g_B_peak_GeO2(m_Ge) : BRIL_G_PEAK
